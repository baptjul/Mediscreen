...
public class AssessmentServiceTests {
    ...
    public void testAssessRisks() {
        ...
        long triggerCount = histories.stream()
                .filter(historyItem -> historyItem.getNote() != null)
                .flatMap(historyItem -> assessmentService.triggers.stream().filter(trigger -> historyItem.getNote().toLowerCase().contains(trigger.toLowerCase())))
                .count();

        String expectedRisk = assessmentService.calculateRiskLevel(age, patient.getSex(), (int) triggerCount);
